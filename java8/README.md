...
- 我们可以在一个接口上使用**@Functionallnterface**注解，这样做可以检查它是否是一个函数式接口。同时javadoc也会包含一条声明，说明这个接口是一个函数式接口。
...
```java
@FunctionalInterface
public interface MyFactionalInterface {
    void method();
}
```
...
- 情况三要求：当函数式接口方法的第一个参数是需要引用方法的调用者，并且第二个参数是需要引用方法的参数（或无参数）时使用
...
| mapToDouble(ToDoubleFunction f) | 接收一个函数作为参数，该函数会被应用到每个元素上，产生一个新的DoubleStream。接收一个函数作为参数，该函数会被应用到每个元素上，产生一个新的IntStream。 |
...
如果开发中需要定义一个函数式接口，首先看看在已有的jdk提供的函数式接口是否提供了能满足需求的函数式接口。如果有，直接调用即可。

### 何时使用方法引用与构造器引用？

如果给定的函数式接口提供实例，恰好满足方法引用的使用情景。